...
from clipwise_detector import PANNClipwiseDetector  # Import our new detector
from Qwen_processor import QwenAudioDescription


def parse_args():
    parser = argparse.ArgumentParser(description='PANN Clipwise Pattern Change Detection')

    # Basic audio processing arguments (matching reference script)
    ...
    parser.add_argument("--fmin", type=int, default=50, help="Minimum frequency")
    ...
    parser.add_argument("--similarity_metric", type=str, default="cosine",
                        choices=["cosine", "kl_divergence", "js_divergence"],
                        help="Similarity metric: cosine or kl_divergence or js divergence")
    parser.add_argument("--cosine_threshold", type=float, default=0.9,
                        help="Threshold for cosine similarity detection")
    parser.add_argument("--kl_threshold", type=float, default=10.0,
                        help="Threshold for KL divergence similarity detection")
    parser.add_argument("--js_threshold", type=float, default=10.0,
                        help="Threshold for JS divergence similarity detection")
...
def process_audio(test_audio_path):
    """Main audio processing function using PANN clipwise detector"""
    # Initialize models and queues fresh for each file
    args = parse_args()

    # Determine threshold based on similarity metric
    if args.similarity_metric == "cosine":
        similarity_threshold = args.cosine_threshold
    elif args.similarity_metric == "kl_divergence":  # kl_divergence
        similarity_threshold = args.kl_threshold
    else:
        similarity_threshold = args.js_threshold
    ...
    clipwise_detector = PANNClipwiseDetector(args,
    ...
    try:
        ...
        print(f"Using {args.similarity_metric} similarity with threshold {similarity_threshold}")
        ...
        result = {
            ...
            "detection_method": f"PANN_Clipwise_{args.similarity_metric}",
            "similarity_metric": args.similarity_metric,
            "similarity_threshold": similarity_threshold,
            "initial_segment": {},
            "pattern_changes": [],
        ...
        if n_chunks < 15:
            ...
            clipwise_detector.prev_clipwise = None
            ...
            for chunk_index in range(15, n_chunks):
                ...
                if chunk_index >= 18:  # We need at least 18 seconds of data for the first 4-second window starting at 15
                    ...
                    # Calculate similarity using PANN clipwise output
                    if clipwise_detector.prev_clipwise is None:
                        _, clipwise_output = clipwise_detector.detect_pattern_change(detection_audio_path)
                        clipwise_detector.prev_clipwise = clipwise_output
                    else:
                        similarity, clipwise_output = clipwise_detector.detect_pattern_change(detection_audio_path)

                        # Check if pattern change detected
                        if similarity < clipwise_detector.similarity_threshold:
                            print(f"Pattern change detected! {args.similarity_metric} similarity: {similarity:.4f}")
                        ...
                        # Update clipwise output for next comparison
                        clipwise_detector.prev_clipwise = clipwise_output
...
def main():
    ...
    # Determine threshold for logging
    if args.similarity_metric == "cosine":
        threshold = args.cosine_threshold
    elif args.similarity_metric == "kl_divergence":
        threshold = args.kl_threshold
    else:  # js_divergence
        threshold = args.js_threshold

    print(f"Starting batch processing with {args.similarity_metric} similarity (threshold: {threshold})")
    ...
    if all_results:
        json_output = {
            "dataset_info": {
                ...
                "pann_checkpoint": args.checkpoint_path
            },
            "results": all_results
        }

        # Create filename based on similarity metric
        json_filename = f"pann_clipwise_{args.similarity_metric}_results.json"
        ...
        try:
            ...
            print(f"Detection method: PANN Clipwise {args.similarity_metric} (threshold: {threshold})")
...
def main_single_test():
    ...
    # Test file
    test_file = "/home/zhyuan/Desktop/Qwen-Audio/ambisonics_audio/R0056_segment_ambisonics.wav"
    results_path = "/home/zhyuan/Desktop/Qwen-Audio/results"
    ...
    if args.similarity_metric == "cosine":
        ...
        threshold = args.js_threshold
    print(f"Testing single file with {args.similarity_metric} similarity (threshold: {threshold})")

    file_data = process_audio(test_file)
    ...
    if file_data:
        ...
        json_filename = f"single_test_pann_clipwise_{args.similarity_metric}.json"
        json_path = os.path.join(results_path, json_filename)

        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(json_output, f, indent=2, ensure_ascii=False)

        print(f"Single test result saved to {json_path}")
        print(f"Changes detected: {len(file_data['pattern_changes'])} at positions {file_data['change_points']}")
        print(f"Time sent to Qwen: {file_data['time_length_sent_to_qwen']} seconds")


if __name__ == "__main__":
    # For batch processing, use main()
    # main()

    # For single file testing, uncomment the line below:
    main_single_test()
